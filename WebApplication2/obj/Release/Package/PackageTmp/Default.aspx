    ...
    */
    ...
    function formatLine(points) {
      ...
      for (var i = 0; i < points.length; i++) {
        if (i == 0)
          pntString = pntString + '[' + points[i].lng + ',' + points[i].lat + ']';
        else {
          pntString = pntString + ', [' + points[i].lng + ',' + points[i].lat + ']';
        }
      }
      ...
      $.ajax({
        ...
        success: function (data) {
          ...
          if (data.d.length > 0) {
            ...
            traceLine = map.getSource('src_trace');
            if (traceLine == null) {
              ...
              traceLine.setData(formatLine(tracePins));
            }
        ...
        success: function (data) {
          ...
          for (var j = 0; j < data.d.length; j++) {
            ...
            for (var i = 0; i < data.d[j].Points.length; i++) {
              ...
              var trackLocation = new Microsoft.Maps.Location(lat, lng);
              coords[i] = trackLocation;

              var location = new Microsoft.Maps.Location(lat, lng);
              var icon = 'images/Misc/Circle_Blue_8x8.png';
              var HW = 8;
              var anchor = new Microsoft.Maps.Point(4, 4);
              var pin;
              if (i == 0) {
                icon = 4;
                HW = 32;
              ...
              else if (i == data.d[j].Points.length - 1) {
                icon = 'images/Misc/EventEnd32x32.png';
                HW = 32;
                anchor = new Microsoft.Maps.Point(16, 32);
                pin = new Microsoft.Maps.Pushpin(location, {
                  draggable: false,
              ...
              pin.metadata = {
                title: time,
                description: 'Speed: ' + speed + '<br/>State: ' + state + '<br/>Latitude: ' + lat + '<br/>Longitiude: '
                  + lng + '<br/>Position Time: ' + '<br/>' + time
              };
              Microsoft.Maps.Events.addHandler(pin, 'mouseover', traceHover);
              Microsoft.Maps.Events.addHandler(pin, 'mouseout', traceUnhover);
              pins[i] = pin;
            }
            ...
            Microsoft.Maps.Events.addHandler(line, 'mouseover', lineHover);
            Microsoft.Maps.Events.addHandler(line, 'mouseout', lineUnhover);
        ...
        success: function (data) {
          ...
          for (var j = 0; j < data.d.length; j++) {
            ...
            for (var i = 0; i < data.d[j].Points.length; i++) {
              ...
              var pin;
              if (i == 0) {
                icon = 4;//'images/Misc/EventStart32x32.png';
                HW = 32;
                anchor = new Microsoft.Maps.Point(16, 32);
                pin = new Microsoft.Maps.Pushpin(location, {
                  draggable: false,
